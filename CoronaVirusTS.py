# -*- coding: utf-8 -*-
"""
Created on Sun Jan 12 15:11:52 2020
...
df = pd.read_excel('CleanedCoronaVirus.xlsx')
fpdf=pd.DataFrame()

df.head(5)

#Define Y column
#fpdf=pd.DataFrame(df(['datetime'],['likes_counts']))
"""
fpdf['datetime']=df['datetime']
fpdf['y']=df['likes_count']
fpdf['date']=df['datetime']
"""

fpdf=df.loc[:,('datetime','likes_count')]




fpdf = fpdf.set_index('datetime')

fpdf=fpdf.resample('D').apply({'y':'sum'})
#e carrying capacity---> maximum size of each day
#df['cap']=50
fpdf=fpdf.reset_index()


fpdf = fpdf.rename(columns={'datetime': 'ds',
                        'y': 'y'})

fpdf.head(5)

fpdf.plot(x='ds',y='y',kind='hist')
#df = df.set_index('ds')
...
my_model = Prophet.Prophet(interval_width=0.95,weekly_seasonality=True,daily_seasonality=True)
...
"""
writer=pd.ExcelWriter("C:/Users/session1/Desktop/topup.xlsx",engine='xlsxwriter')
